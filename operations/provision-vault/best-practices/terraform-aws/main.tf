...
data "template_file" "bastion_user_data" {
  template = file(
    "${path.module}/../../templates/best-practices-bastion-systemd.sh.tpl",
  )
...
data "template_file" "consul_user_data" {
  template = file(
    "${path.module}/../../templates/best-practices-consul-systemd.sh.tpl",
  )
...
module "consul_aws" {
  ...
  subnet_ids = split(
    ",",
    var.consul_public ? join(",", module.network_aws.subnet_public_ids) : join(",", module.network_aws.subnet_private_ids),
  )
...
data "template_file" "vault_user_data" {
  template = file(
    "${path.module}/../../templates/best-practices-vault-systemd.sh.tpl",
  )
...
module "vault_aws" {
  ...
  subnet_ids = split(
    ",",
    var.vault_public ? join(",", module.network_aws.subnet_public_ids) : join(",", module.network_aws.subnet_private_ids),
  )
